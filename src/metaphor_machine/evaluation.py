...
def evaluate_problem(
    ...
    *,
    ...
    results: list[ComparisonResult] = pipe.run_judge_batch(
        n=runs, baseline_text=baseline, base_seed=base_seed
    )
    return {
...
def run_evaluation(
    ...
    *,
    ...
    Args:
        ...
        progress: optional callback ``fn(done, total, label)`` for UI progress.
    ...
    for i, p in enumerate(problems):
        ...
        try:
            ...
            per_problem.append({"id": pid, "category": category, "error": str(exc)})
            continue
